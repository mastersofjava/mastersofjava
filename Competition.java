...
public class Competition {
	...
	public boolean initAssignments() {
		...
		try {
			Files.walk(assignmentPath, 2).filter(f -> f.endsWith("pom.xml")).forEach(f -> {
				try {
					String filename = f.getFileName().toString();
					String fileAndDir = f.toString();
					int beginIndex = fileAndDir.indexOf(assignmentDirectory) + assignmentDirectory.length() + 1;
					...
					addAssignmentFile(new AssignmentFile(f.getFileName().toString(), content, FileType.POM, assignment, f.toFile()));
				} catch (IOException e) {
					e.printStackTrace();
				}
			});
...
//        timeHandler = scheduledExecutorService.scheduleAtFixedRate(
            () -> {
                ...
                new Date(System.currentTimeMillis() + (solutiontime * 3000))
                );
...
//        threadPoolTaskScheduler.schedule(
//                () -> soundService.playTicTac(secondsBeforeEnd),
//                new Date(System.currentTimeMillis + ((solutiontime - secondsBeforeEnd) * 3000))
//
//                new Runnabletask("Specific time, 3 Seconds from now"),
//                new Date(System.currentTimeMillis + 3000)
//        );
	...
	private void startAssignmentRunnable(final Assignment assignment, Integer solutiontime) {
		...
		assignmentHandler = scheduledExecutorService.schedule(() -> {
		    log.info("Stopping assignment because of solutiontime: " + solutiontime);
